...
def test_linear_plus_sinusoid_by_hand():
    ...
    tc = model(p, planet, epoch, 2, LET, False)
...
def test_phase_from_linear_ephemeris_not_perturbed():
    ...
    p, (planet, epoch) = params(), data()
    base = model(dict(p, as_bc=0.0, ac_bc=0.0), planet, epoch, 2, LET, False)
    d1 = model(dict(p, as_bc=0.008, ac_bc=0.0), planet, epoch, 2, LET, False) - base
    d2 = model(dict(p, as_bc=0.016, ac_bc=0.0), planet, epoch, 2, LET, False) - base
...
def test_jacobian_matches_numeric(phase_offsets, nto):
    ...
    names = sorted(p)
    J = jacobian(p, names, planet, epoch, 2, LET, nto, phase_offsets)
    for k, nm in enumerate(names):
        h = 1e-6 * max(1.0, abs(p[nm]))
        hi = model(dict(p, **{nm: p[nm]+h}), planet, epoch, 2, LET, nto, phase_offsets)
        lo = model(dict(p, **{nm: p[nm]-h}), planet, epoch, 2, LET, nto, phase_offsets)
...
def test_three_planet_middle_gets_both_pairs():
    ...
    tc = model(p, planet, epoch, 3, 'bcd', False)
...
def test_time_shift_invariance_with_tref():
    ...
    p, (planet, epoch) = params(), data()
    base = model(p, planet, epoch, 2, LET, False)
    S = 2454833.0
    p2 = dict(p, t0_b=p['t0_b'] + S, t0_c=p['t0_c'] + S)
    shifted = model(p2, planet, epoch, 2, LET, False, t_ref=S)
...
def test_jacobian_shift_invariance_with_tref():
    ...
    p, (planet, epoch) = params(), data()
    names = sorted(p)
    J0 = jacobian(p, names, planet, epoch, 2, LET, False)
    S = 2454833.0
    p2 = dict(p, t0_b=p['t0_b'] + S, t0_c=p['t0_c'] + S)
    J1 = jacobian(p2, names, planet, epoch, 2, LET, False, t_ref=S)
